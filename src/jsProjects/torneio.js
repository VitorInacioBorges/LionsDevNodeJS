...
async function removerTorneio() {
    ...
    const input = await askQuestion('Digite o ID do torneio que quer remover: ');
    if (torneios.length == 0) {
        ...
        if(input<0 || input > torneios.length || isNaN(input)){
            ...
            let [removido] = torneios.splice(input, 1);
            console.log('Removido com sucesso! Pressione ENTER para voltar ao menu...');
            rl.question('', menuPrincipal);
        }
    }
    menuPrincipal();
